...
impl TransferEngine {
    ...
    pub fn execute(
        ...
        for dir_node in &selected_dirs {
            ...
            if has_tar {
                ...
                match TarPuller::pull_dir(client, &dir_node.path, local_dir.to_str().unwrap_or(""), &self.progress) {
                    ...
                    Err(e) => {
                        // Fallback to individual file pull
                        let mut progress = self.progress.lock().unwrap();
                        progress.errors.push((dir_node.path.clone(), format!("Tar failed, falling back: {}", e)));
                        drop(progress);

                        self.pull_files_individually(client, &dir_node.selected_files(), destination, state_manager)?;
                    }
                }
            } else {
                // No tar available, pull files individually
                self.pull_files_individually(client, &dir_node.selected_files(), destination, state_manager)?;
            }
        }
        ...
        if !standalone_files.is_empty() {
            self.pull_files_individually(client, &standalone_files, destination, state_manager)?;
        }
    ...
    /// Pull files one by one with progress tracking.
